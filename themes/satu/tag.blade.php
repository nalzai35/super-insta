@extends('satu.layouts')

@php
    $imoticon = ['😀', '😂', '👌', '❤️', '😍', '🔥', '⚡️', '🚀', '🌄', '❤️', '💚', '💕', '💓', '💖', '🟢', '🔴', '✅', '💰', '💎', '🩸', '⛺️', '🥇', '🥉', '🏆', '🎨', '🎯', '🏅', '🥈', '🌈'];
    $imoticon = collect($imoticon)->shuffle()->take(5)->implode('');
@endphp

@section('head')
    <title>{{ $imoticon }} #{{ $tag }} Instagram hashtag Photos & Videos • {{ ucwords($site_name) }}</title>
    <meta name="description" content="Check out Instagram photos, videos and stories about hashtag #{{ $tag }}.">
@endsection

@section('content')
    <div class="container py-5">
        <!--ads/responsive-->

        <h1>{{ ucwords($tag) }}</h1>

        <div class="row">
            @foreach ($file as $key => $data)
                <div class="col-md-4 mb-3">
                    <div class="card">
                        <img src="{{ $data['image'] }}" class="card-img-top" alt="{{ $data['caption'] }}">
                        <div class="card-body">
                            <p class="card-text small text-muted" style="max-height: 5rem; overflow: auto;">{{ $data['caption'] }}</p>
                        </div>
                    </div>
                </div>
                @if ($key % 6 == 1)
                    <div class="col-md-4 mb-3">
                        <!--ads/responsive-->
                    </div>
                @endif
            @endforeach
        </div>
    </div>

    <div class="container py-5">
        <div class="text-center">
            <h2>Trending Hashtags</h2>
            <div class="mt-4">
                <ul class="list-inline">
                    @foreach (collect($all_files)->shuffle()->take(33) as $item)
                        <li class="list-inline-item py-1 px-2 rounded bg-light mb-3 border">
                            <a href="{{ str_replace('.srz.php', '', $item) }}.html">#{{ str_replace('.srz.php', '', $item) }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
        </div>
    </div>

    <footer class="footer border-top bg-light py-3">
        <div class="container">
            <ul class="list-inline m-0 text-center">
                @foreach($pages as $page)
                    <li class="list-inline-item">
                        <a class="text-muted" href="{{ $page . '.html' }}">{{ ucwords(str_replace('-', ' ', $page)) }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
    </footer>
@endsection
